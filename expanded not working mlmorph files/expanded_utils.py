...
def generate_dlib_xml(images,sizes,folder='train',copy_img,out_file='output.xml'):
    '''
    ...
    Parameters:
        ...
        copy_img(bool)= are images being copied to a new directory?
        out_file(str)= prefix for xmlfile
...
def split_train_test(input_dir,random_seed,copy_img,split_img,out_file):
    '''
    ...
    if split_img:
        ...
        filenames = {out_file+'train':train_set,
                    out_file+'test': test_set}
        inter_list = [out_file+'train',out_file+'test']
    else:
        train_set = filenames
        filenames = {out_file+'train':train_set}
        inter_list = [out_file+'train',out_file+'test']
    ...
    for split in inter_list:
        ...
        if not os.path.exists(split):
            ...
            os.mkdir(split) # comment out to avoid deleting train and test if they are already there

        for filename in filenames[split]:
            basename=os.path.basename(filename)
            name=os.path.splitext(basename)[0] + '.jpg'
            sizes[split][name]=image_prep(filename,name,split,copy_img)
    return sizes

def image_prep(file,name,dir_path,copy_img):
    '''
    ...
    Parameters:
        ...
        copy_img(bool)=should the image be copied to a new directory?
        dir_path(str)= directory where the image file should be saved to if copy_img=True
