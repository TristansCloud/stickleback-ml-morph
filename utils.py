...
def subset_generate_dlib_xml(images,sizes,folder='train',out_file='output.xml'):
    '''
    ...
    for i in range(0,len(images['im'])):
        ...
        if os.path.isfile(path) is True: 
            ...
            if path in present_tags:
                ...
                images_e[pos].append(add_bbox_element(images['coords'][i],sizes[path]))

            else:    
                images_e.append(add_image_element(name,images['coords'][i],sizes[path],path))
...
def train_on_subset_img_prep(input_dir):
    '''
    Internal function used by train_on_subset.py. Reads the images
    in a folder and finds the image dimensions.
